...
export function EndView({ icon, title, sub, items, answers, taskString, scores, onNext, hideNextButton }: EndViewProps) {
  ...
  return (
    <div className="animate-fade-in text-center pt-8">
      ...
      <div className="text-xl font-bold text-foreground mb-1.5">{title}</div>
      {answers && <span className="text-[10px] font-bold px-2.5 py-1 rounded-full border" style={{ background: leadC + '10', color: leadC, borderColor: leadC + '25' }}>{t[ll]} LEAD — {ls}pts</span>}
      <div className="text-sm text-muted-foreground max-w-[380px] mx-auto mt-2 mb-4 leading-relaxed">{sub}</div>
      {items && (
        <div className="inline-flex flex-col gap-1 text-left bg-card rounded-xl px-5 py-3 mb-3 border border-border shadow-sm">
          {items.map((d, i) => <div key={i} className="text-xs text-muted-foreground">✓ {d}</div>)}
        </div>
      )}
      {answers && (
        <div className="inline-flex flex-col gap-0.5 text-left bg-primary/[0.04] border border-primary/15 rounded-xl px-5 py-3 mb-3">
          <div className="text-[10px] font-bold text-primary uppercase tracking-wider mb-1">{t.surveySummary}</div>
          <div className="text-xs text-muted-foreground">{t.hoursWeek}: {answers.hours || '-'}</div>
          <div className="text-xs text-muted-foreground">{t.tasks}: {taskString || '-'}</div>
          <div className="text-xs text-muted-foreground">{t.growthPhase}: {answers.growth || '-'}</div>
          <div className="text-xs text-muted-foreground">{t.aiStatus}: {answers.ai || '-'}</div>
        </div>
      )}
      {scores && scores.gebeld > 0 && (
        <div className="inline-grid grid-cols-4 gap-3 bg-card rounded-xl px-5 py-3 mb-3 border border-border shadow-sm">
          {[
            ...
            { l: t.conversion, v: conv + '%', c: conv >= 50 ? 'hsl(var(--success))' : 'hsl(var(--warning))' },
          ].map(x => (
