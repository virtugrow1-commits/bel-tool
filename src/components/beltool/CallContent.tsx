...
export function CallContent({
  ...
  // Precall screen — shows last status of this lead + call button
  if (phase === 'precall') {
    const stageLabels: Record<string, { label: string; icon: string; color: string }> = {
      nieuw: { label: 'Nieuw contact', icon: '🆕', color: 'hsl(var(--primary))' },
      bellen: { label: 'In bellijst', icon: '📋', color: 'hsl(var(--primary))' },
      geenGehoor: { label: 'Geen gehoor', icon: '📵', color: 'hsl(var(--muted-foreground))' },
      nietInteressant: { label: 'Niet geïnteresseerd', icon: '❌', color: 'hsl(var(--destructive))' },
      terugbellenGepland: { label: 'Terugbellen gepland', icon: '📅', color: 'hsl(var(--warning))' },
      anderMoment: { label: 'Ander moment', icon: '⏳', color: 'hsl(280 60% 55%)' },
      enqueteTel: { label: 'Enquête voltooid', icon: '✅', color: 'hsl(var(--success))' },
      enqueteVerstuurd: { label: 'Digitaal verstuurd', icon: '📨', color: 'hsl(265 83% 57%)' },
      afspraak: { label: 'Afspraak gepland', icon: '🤝', color: 'hsl(var(--success))' },
    };
    const stageInfo = stageLabels[activeComp.stage] || { label: activeComp.stage, icon: '📋', color: 'hsl(var(--primary))' };
      ...
      <div className="flex-1 flex flex-col bg-background">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 px-6">
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="px-4 py-1.5 rounded-full text-[13px] font-bold border" style={{ borderColor: stageInfo.color, color: stageInfo.color, backgroundColor: `color-mix(in srgb, ${stageInfo.color} 10%, transparent)` }}>
              {stageInfo.label}
            </div>
          </div>
          {activeContact.notes && (
            <div className="bg-card border border-border rounded-xl p-4 max-w-md w-full">
              <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1.5">Laatste notities</div>
              <div className="text-[13px] text-foreground/80 whitespace-pre-wrap">{activeContact.notes}</div>
            </div>
          )}
          <div className="flex flex-col items-center gap-3 w-full max-w-xs">
            <CallButton
              ...
              onCallStarted={(callId) => { setPhase('intro'); onStartDialing(callId); }}
            ...
            >
              Ga naar script zonder te bellen
            </button>
          </div>
        </div>
      </div>
